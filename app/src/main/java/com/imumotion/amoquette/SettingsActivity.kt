...
class SettingsActivity : AppCompatActivity() {
    ...
    class SettingsFragment : PreferenceFragmentCompat() {
        override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
            ...
            portPreference?.summaryProvider = EditTextPreferenceHelper(portPreference, true)

            // Wake lock duration
            val wakelockPreference: EditTextPreference? =
                findPreference(BROKER_PROPERTY_WAKE_LOCK_DURATION)
            wakelockPreference?.summaryProvider = EditTextPreferenceHelper(wakelockPreference, true)

/*            // Message size
            val msgsizePreference: EditTextPreference? =
                findPreference(BROKER_PROPERTY_MESSAGE_SIZE)
            msgsizePreference?.summaryProvider = EditTextPreferenceHelper(msgsizePreference, true)*/

            // $SYS messages interval
            val sysPreference: EditTextPreference? =
                findPreference(BROKER_PROPERTY_SYSMSG_INTERVAL)
            sysPreference?.summaryProvider = EditTextPreferenceHelper(sysPreference, true)
            ...
            //User name
            val userPreference: EditTextPreference? = findPreference(BROKER_USERNAME)
            userPreference?.summaryProvider = EditTextPreferenceHelper(userPreference)

            //Password
            val passwordPreference: EditTextPreference? = findPreference(BROKER_PASSWORD)
            passwordPreference?.summaryProvider = EditTextPreferenceHelper(passwordPreference)
        }
    }
